...
export class GoogleReviewRepository
  extends BaseRepository<GoogleReview, string>
  implements IReviewRepository<GoogleReviewWithMetadata>
{
  protected model = prisma.googleReview;

  async findByBusinessId(
    businessId: string,
  ): Promise<GoogleReviewWithMetadata[]> {
    return (await this.model.findMany({
      where: { businessProfileId: businessId },
    })) as GoogleReviewWithMetadata[];
  }

  async findByBusinessIdWithMetadata(
    businessId: string,
  ): Promise<GoogleReviewWithMetadata[]> {
    return (await this.model.findMany({
      where: { businessProfileId: businessId },
  ...
  async create(
    review: Omit<GoogleReviewWithMetadata, "id" | "createdAt" | "updatedAt">,
  ): Promise<GoogleReviewWithMetadata> {
  ...
  async createMany(
    reviews: Omit<GoogleReviewWithMetadata, "id" | "createdAt" | "updatedAt">[],
  ): Promise<GoogleReviewWithMetadata[]> {
    ...
    return await this.findByBusinessIdWithMetadata(
      reviews[0]?.businessProfileId || "",
    );
  }

  async update(
    id: string,
    review: Partial<GoogleReviewWithMetadata>,
  ): Promise<GoogleReviewWithMetadata> {
    const { reviewMetadata, ...reviewData } = review;
