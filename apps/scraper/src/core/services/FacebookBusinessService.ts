...
import type { FacebookBusinessProfile } from "@prisma/client";
...
export class FacebookBusinessService implements IBusinessService {
  ...
  async createProfile(
    ...
    try {
      ...
      const profile = await this.businessRepository.findById(
  ...
  async getProfile(
    ...
    try {
      const profile = await this.businessRepository.findByPlatform(
        teamId,
        platform,
      );
