...
export class UnifiedBusinessService {
  ...
  constructor(private container: IDependencyContainer) {
    this.taskTracker = this.container.getService<ITaskTracker>(
  ...
  async createBusinessProfile(
    ...
    try {
      ...
      await this.taskTracker.startStep(
        ...
        `Creating ${platform} business profile...`,
      );

      const serviceToken = this.getBusinessServiceToken(platform);
      const businessService =
        this.container.getService<IBusinessService>(serviceToken);
      ...
      if (result.success) {
        await this.taskTracker.completeStep(
          ...
          platform,
          TaskStep.CREATING_PROFILE,
          `${platform} business profile created successfully`,
          result,
        );
      } else {
      ...
      await this.taskTracker.failStep(
        ...
        platform,
        TaskStep.CREATING_PROFILE,
        `Error creating business profile: ${error}`,
      );
      throw error;
    }
  }
  ...
  async saveBusinessReviews(
    ...
    const reviewService =
      this.container.getService<IReviewService<any>>(serviceToken);
  ...
  async processBusinessAnalytics(
    ...
    const analyticsService =
      this.container.getService<IAnalyticsService>(serviceToken);
  ...
  async triggerReviewScraping(
    ...
    try {
      await this.taskTracker.startStep(
        ...
        `Starting ${platform} review scraping...`,
      );
      ...
      await this.taskTracker.failStep(
        ...
        platform,
        TaskStep.FETCHING_REVIEWS,
        `Error starting review scraping: ${error}`,
      );
      return {
        success: false,
