...
export const bookingBusinessReviewsActorJobRunner = async (
  ...
  try {
    logger.info(
      `📝 Starting Booking.com reviews scraping for URL: ${data.bookingUrl}`,
    );

    if (!data.bookingUrl) {
      throw new Error(
        "Booking URL is required for Booking.com reviews scraping",
      );
    }
    ...
    if (!run || !run.defaultDatasetId) {
      logger.error(
        "❌ Booking.com reviews scraping failed: No dataset ID returned",
      );
      return false;
    }
    ...
    if (!items || items.length === 0) {
      logger.info(
        "📭 No reviews found for Booking.com property:",
        data.bookingUrl,
      );
      return true;
    }
    ...
    logger.info(
      `✅ Booking.com reviews scraping completed successfully. Found ${items.length} reviews. Dataset ID: ${run.defaultDatasetId}`,
    );
    ...
    try {
      ...
      logger.info(
        `📊 Analytics processing completed for businessProfileId: ${businessProfile.id}`,
      );
    } catch (analyticsError) {
      logger.error(
        `❌ Error processing Booking.com analytics:`,
    ...
    logger.error(
      `💥 Booking.com reviews actor job failed for team ${data.teamId}, URL ${data.bookingUrl}: ${errorMessage}`,
    );
    return false;
  }
};
...
export async function handleBookingBusinessReviews(
  ...
  if (data.platform !== "BOOKING" || !data.bookingUrl) {
    logger.error(
      `Invalid job data for Booking.com actor: platform=${data.platform}, hasBookingUrl=${!!data.bookingUrl}`,
    );
    return false;
  }
