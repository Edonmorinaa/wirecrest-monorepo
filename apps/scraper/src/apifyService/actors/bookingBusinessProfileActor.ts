...
export const bookingBusinessProfileActorJobRunner = async (
  ...
  try {
    logger.info(
      `🏨 Starting Booking.com business profile scraping for URL: ${data.bookingUrl}`,
    );

    if (!data.bookingUrl) {
      throw new Error(
        "Booking URL is required for Booking.com profile scraping",
      );
    }
    ...
    if (!run || !run.defaultDatasetId) {
      logger.error(
        "❌ Booking.com profile scraping failed: No dataset ID returned",
      );
      return false;
    }

    logger.info(
      `✅ Booking.com profile scraping completed successfully. Dataset ID: ${run.defaultDatasetId}`,
    );
    ...
    logger.error(
      `💥 Booking.com profile actor job failed for team ${data.teamId}, URL ${data.bookingUrl}: ${errorMessage}`,
    );
    return false;
  }
};
