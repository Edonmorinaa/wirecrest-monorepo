...
export class GoogleBusinessReviewsActorJob {
  ...
  constructor(
    jobPayload: GoogleBusinessReviewsActorJobPayload,
    apifyToken: string /*, dbConnectionString: string */,
  ) {
  ...
  public async run(): Promise<void> {
    ...
    console.log(
      `[ActorJob] Starting Google Reviews scrape for placeId: ${placeId}, initialization: ${isInitialization}`,
    );
    ...
    try {
      ...
      if (!businessProfileId) {
        const profile =
          await this.databaseService.getBusinessByPlaceId(placeId);
        if (!profile) {
          console.error(
            `[ActorJob] No GoogleBusinessProfile found for placeId: ${placeId}. Cannot save reviews.`,
          );
        ...
        if (profile.teamId !== teamId) {
          console.error(
            `[ActorJob] Mismatched teamId for placeId ${placeId}. Expected ${teamId}, found ${profile.teamId}.`,
          );
          return;
        }
      ...
      console.log(
        `[ActorJob] Calling Apify actor ${actorId} with input:`,
        input,
      );
      ...
      if (!run || !run.defaultDatasetId) {
        console.error(
          `[ActorJob] Apify actor run failed or did not produce a dataset for placeId: ${placeId}.`,
        );
        throw new Error("Apify actor run failed for Google Reviews.");
      }

      const { items: reviewsFromApify } = await this.apifyClient
        .dataset(run.defaultDatasetId)
        .listItems();

      if (!reviewsFromApify || reviewsFromApify.length === 0) {
        console.log(
          `[ActorJob] No new reviews found by Apify for placeId: ${placeId}.`,
        );
      ...
      console.log(
        `[ActorJob] Fetched ${reviewsFromApify.length} reviews from Apify for placeId: ${placeId}.`,
      );
      ...
      console.log(
        `[ActorJob] Successfully saved ${reviewsFromApify.length} reviews for placeId: ${placeId}.`,
      );
      ...
      console.error(
        `[ActorJob] Error during Google Reviews job for placeId: ${placeId}:`,
        error,
      );
