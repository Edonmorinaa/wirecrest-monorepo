...
import { StripeFirstSubscriptionService } from '@wirecrest/billing/server-only';
...
export class PlatformConfigWebhookController {
  ...
  async handlePlatformConfigured(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!subscription) {
        console.log(`⚠️ Team ${teamId} has no active subscription, skipping scraping setup`);
        res.json({
          success: true,
          message: 'Platform configured but no active subscription. Scraping will start when subscription is active.',
      ...
      if (!profileResult.exists) {
        res.status(500).json({
          ...
          message: profileResult.error || 'Unknown error during profile creation',
        });
        return;
      }
      
      console.log(`✅ Business profile ready: ${profileResult.businessProfileId} (created: ${profileResult.created})`);
