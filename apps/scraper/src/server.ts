...
app.post(
  ...
  async (req: Request, res: Response) => {
    try {
      const { locationId, instagramUsername, snapshotTime, timezone } = req.body;

      if (!locationId || !instagramUsername) {
        res.status(400).json({
          success: false,
          error: "locationId and instagramUsername are required",
        });
        return;
      }

      console.log(`📸 Creating Instagram profile for location ${locationId}, username: ${instagramUsername}`);

      // Import Instagram service
      const { InstagramDataService } = await import("./services/instagramDataService");
      ...
      const profileResult = await instagramService.createBusinessProfile(
        locationId,
        instagramUsername
      );
      ...
      if (!profileResult.success) {
        res.status(400).json({
          success: false,
          error: profileResult.error || "Failed to create Instagram profile",
        });
        return;
      }

      // Optionally enable automatic snapshots
      if (profileResult.businessProfileId && snapshotTime) {
        await instagramService.enableAutomaticSnapshots(
...
app.post(
  "/api/instagram/snapshot",
  async (req: Request, res: Response) => {
    try {
      const { businessProfileId, instagramUsername } = req.body;

      if (!businessProfileId || !instagramUsername) {
        res.status(400).json({
          success: false,
          error: "businessProfileId and instagramUsername are required",
        });
        return;
      }

      console.log(`📸 Taking Instagram snapshot for profile ${businessProfileId}`);
      ...
      // Take snapshot
      const snapshotResult = await instagramService.takeSnapshot(
        businessProfileId,
        instagramUsername
      );
      ...
      res.json({
        ...
        message: "Instagram snapshot taken successfully",
      });
    } catch (error: any) {
