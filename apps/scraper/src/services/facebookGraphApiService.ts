...
export class FacebookGraphApiService {
  ...
  constructor() {
    ...
    if (!this.accessToken) {
      throw new Error(
        "Facebook access token is required (FACEBOOK_ACCESS_TOKEN environment variable)",
      );
    }
  }
  ...
  async getPageData(pageIdentifier: string): Promise<FacebookGraphApiProfile> {
    try {
      logger.info(
        `[Facebook Graph API] Fetching page data for: ${pageIdentifier}`,
      );
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Facebook Graph API error: ${response.status} - ${errorData.error?.message || response.statusText}`,
        );
      }

      const data = (await response.json()) as FacebookGraphApiProfile;
      logger.info(
        `[Facebook Graph API] Successfully fetched data for page: ${data.name} (${data.id})`,
      );

      return data;
    } catch (error) {
      logger.error(
        "[Facebook Graph API] Error fetching page data:",
        error as any,
      );
      throw error;
    }
  ...
  ): Promise<FacebookGraphApiInsights> {
    try {
      ...
      if (!response.ok) {
        ...
        logger.error(
          "[Facebook Graph API] Insights error response:",
          undefined,
          errorInfo,
        );
        ...
        if (response.status === 400 || response.status === 403) {
          logger.warn(
            "[Facebook Graph API] Insights not available for this page, returning empty data",
          );
          return {};
        }

        throw new Error(
          `Facebook Graph API insights error: ${response.status} - ${errorData.error?.message || response.statusText}`,
        );
      }
      ...
      if (data.data && Array.isArray(data.data)) {
        data.data.forEach((metric: any) => {
          ...
          if (values && values.length > 0) {
            insights[name as keyof FacebookGraphApiInsights] =
      ...
      logger.info(
        `[Facebook Graph API] Successfully fetched insights for page: ${pageId}`,
      );
      return insights;
    } catch (error) {
  ...
  async searchPages(
    query: string,
    limit: number = 25,
  ): Promise<FacebookGraphApiProfile[]> {
    try {
      ...
      if (!response.ok) {
        ...
        logger.error(
          "[Facebook Graph API] Search error response:",
          undefined,
          errorInfo,
        );
        throw new Error(
          `Facebook Graph API search error: ${response.status} - ${errorData.error?.message || response.statusText}`,
        );
      }
      ...
      logger.info(
        `[Facebook Graph API] Found ${pages.length} pages for query: ${query}`,
      );
      return pages;
    } catch (error) {
  ...
  static extractPageIdFromUrl(facebookUrl: string): string | null {
    try {
      ...
      logger.error(
        "[Facebook Graph API] Error extracting page ID from URL:",
        error,
      );
      return null;
    }
  ...
  async validateAccessToken(): Promise<boolean> {
    try {
      ...
      logger.error(
        "[Facebook Graph API] Error validating access token:",
        error,
      );
      return false;
    }
  ...
  static convertToBusinessProfile(
    ...
    return {
      ...
      categories:
        graphData.category_list?.map((cat) => cat.name) ||
        (graphData.category ? [graphData.category] : []),
      info: [
        graphData.about,
