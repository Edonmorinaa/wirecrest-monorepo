...
export class DataCleanupService {
  ...
  async cleanupTeamPlatformData(
    ...
    try {
      console.log(`[CLEANUP] Starting cleanup for team ${teamId}, platform ${platform}, identifier: ${oldIdentifier}`);
      ...
      console.log(`[CLEANUP] Cleanup completed. Deleted: ${deletedCounts.businessProfiles} profiles, ${deletedCounts.reviews} reviews, ${deletedCounts.overviews} overviews`);
  ...
  private async cleanupGoogleData(
    ...
    try {
      ...
      const { data: businessProfiles, error: businessError } = await this.supabase
      ...
      if (businessError) {
        console.error(`[CLEANUP] Error getting Google business profiles:`, businessError);
      ...
      console.log(`[CLEANUP] Found ${businessProfiles.length} Google business profiles to cleanup for placeId: ${placeId}`);
      console.log(`[CLEANUP] Profile details:`, businessProfiles.map(p => ({ id: p.id, title: p.title })));
      ...
      // Delete overviews
      const { error: overviewsError, count: overviewsCount } = await this.supabase
        .from('GoogleOverview')
        .delete({ count: 'exact' })
        .in('businessProfileId', businessProfileIds);

      if (overviewsError) {
        console.error(`[CLEANUP] Error deleting Google overviews:`, overviewsError);
      } else {
        console.log(`[CLEANUP] Deleted ${overviewsCount || 0} Google overviews`);
      }
      ...
      if (metadataError) {
        console.error(`[CLEANUP] Error deleting Google business metadata:`, metadataError);
      } else {
      ...
      if (profilesError) {
        console.error(`[CLEANUP] Error deleting Google business profiles:`, profilesError);
      } else {
        console.log(`[CLEANUP] Deleted ${profilesCount || 0} Google business profiles`);
      }
  ...
  private async cleanupFacebookData(
    ...
    try {
      ...
      if (businessError) {
        console.error(`[CLEANUP] Error getting Facebook business profiles:`, businessError);
        return { businessProfiles: 0, reviews: 0, overviews: 0 };
      }

      if (!businessProfiles || businessProfiles.length === 0) {
        console.log(`[CLEANUP] No Facebook business profiles found for cleanup`);
        return { businessProfiles: 0, reviews: 0, overviews: 0 };
      }

      const businessProfileIds = businessProfiles.map(profile => profile.id);
      console.log(`[CLEANUP] Found ${businessProfiles.length} Facebook business profiles to cleanup`);
      ...
      if (reviewsError) {
        console.error(`[CLEANUP] Error deleting Facebook reviews:`, reviewsError);
      } else {
      ...
      if (overviewsError) {
        console.error(`[CLEANUP] Error deleting Facebook overviews:`, overviewsError);
      } else {
        console.log(`[CLEANUP] Deleted ${overviewsCount || 0} Facebook overviews`);
      }
      ...
      if (metadataError) {
        console.error(`[CLEANUP] Error deleting Facebook business metadata:`, metadataError);
      } else {
      ...
      if (profilesError) {
        console.error(`[CLEANUP] Error deleting Facebook business profiles:`, profilesError);
      } else {
        console.log(`[CLEANUP] Deleted ${profilesCount || 0} Facebook business profiles`);
      }
