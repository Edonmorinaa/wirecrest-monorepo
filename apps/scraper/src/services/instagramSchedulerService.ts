...
export class InstagramSchedulerService {
  ...
  private async checkAndExecuteScheduledSnapshots(): Promise<void> {
    try {
      ...
      console.log(
        `📅 Checking ${schedules.length} Instagram snapshot schedules...`,
      );
  ...
  private async processSchedule(
    ...
    try {
      ...
      if (
        lastSnapshotTime &&
        now.getTime() - lastSnapshotTime.getTime() < minTimeGap
      ) {
        console.log(
          `⏰ Skipping snapshot for ${schedule.businessProfile.username} - too soon since last snapshot`,
        );
        return;
      }
      ...
      if (existingSnapshot) {
        console.log(
          `✅ Snapshot already exists for ${schedule.businessProfile.username} on ${currentDate}`,
        );
        return;
      }
      ...
      if (todaySnapshots && todaySnapshots.length >= 2) {
        console.log(
          `⚠️  Daily limit reached for ${schedule.businessProfile.username} (${todaySnapshots.length}/2 snapshots)`,
        );
        return;
      }

      console.log(
        `📸 Taking scheduled snapshot for ${schedule.businessProfile.username}...`,
      );
      ...
      // Take the snapshot
      const result = await this.instagramService.takeDailySnapshot(businessId, {
        snapshotType: "DAILY",
        includeMedia: true,
      ...
      if (result.success) {
        ...
        console.log(
          `✅ Successfully took snapshot ${result.snapshotId} for ${schedule.businessProfile.username}`,
        );
      } else {
        ...
        console.error(
          `❌ Failed to take snapshot for ${schedule.businessProfile.username}: ${result.error}`,
        );
        ...
        if (newFailureCount >= schedule.maxRetries) {
          ...
          console.warn(
            `⚠️  Deactivated schedule for ${schedule.businessProfile.username} due to ${newFailureCount} consecutive failures`,
          );
        }
      }
    } catch (error) {
  ...
  async triggerAllSnapshots(): Promise<{
    ...
    try {
      ...
      console.log(
        `🔄 Manually triggering ${schedules.length} Instagram snapshots...`,
      );
      ...
      for (const schedule of schedules) {
        try {
          ...
          if (result.success) {
            processed++;
            console.log(
              `✅ Manual snapshot successful for business ${schedule.businessProfileId}`,
            );
          } else {
            errors++;
            console.error(
              `❌ Manual snapshot failed for business ${schedule.businessProfileId}: ${result.error}`,
            );
          }
        } catch (error) {
          errors++;
          console.error(
            `❌ Error taking manual snapshot for business ${schedule.businessProfileId}:`,
            error,
          );
        }
  ...
  async cleanupOldSnapshots(
    ...
    try {
      ...
      const { data: deletedSnapshots, error: snapshotError } =
        await this.supabase
          .from("InstagramDailySnapshot")
          .delete()
      ...
      console.log(
        `🧹 Cleaned up ${deletedCount} Instagram snapshots older than ${olderThanDays} days`,
      );
