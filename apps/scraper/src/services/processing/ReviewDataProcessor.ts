 ...
 * Now with Zod validation for data integrity
 */
...
export class ReviewDataProcessor {
  ...
  private async sendReviewNotificationToTeam(
    ...
    if (!shouldNotify) {
      console.log(
        `  ℹ️  No notification needed for team ${teamId} - no new reviews`,
      );
      return;
    }
    ...
    try {
      ...
      console.log(
        `  ✉️  Notification sent to team ${teamId}: ${result.reviewsNew} new reviews`,
      );
    } catch (error) {
   ...
   *
   * NOW WITH ZOD VALIDATION: Validates all input data before processing
   */
  async processReviews(
    ...
    // ⚡ VALIDATION: Validate and sanitize input data before processing
    ...
    switch (platform) {
      ...
      case "facebook": {
        const result = validateFacebookReviewsSafe(rawData);
        validatedData = result.valid;
        invalidCount = result.invalid;
        break;
      }
      case "tripadvisor": {
        const result = validateTripAdvisorReviewsSafe(rawData);
        validatedData = result.valid;
        invalidCount = result.invalid;
        break;
      }
      case "booking": {
        const result = validateBookingReviewsSafe(rawData);
        validatedData = result.valid;
        invalidCount = result.invalid;
        break;
      }
    ...
    if (invalidCount > 0) {
      console.warn(
        `⚠️  Validation: ${invalidCount} invalid reviews filtered out`,
      );
    }
    console.log(
      `✅ Validation: ${validatedData.length} valid reviews to process`,
    );
    ...
    switch (platform) {
      case "google_reviews":
        baseResult = await this.processGoogleReviews(
          teamId,
          validatedData,
          isInitial,
        );
        break;
      case "facebook":
        baseResult = await this.processFacebookReviews(
          teamId,
          validatedData,
          isInitial,
        );
        break;
      case "tripadvisor":
        baseResult = await this.processTripAdvisorReviews(
          teamId,
          validatedData,
          isInitial,
        );
        break;
      case "booking":
        baseResult = await this.processBookingReviews(
          teamId,
          validatedData,
          isInitial,
        );
    ...
    if (teamId) {
      await this.sendReviewNotificationToTeam(
        teamId,
        platform,
        baseResult,
        isInitial,
  ...
  private async processGoogleReviews(
    ...
    const uniquePlaceIds = [
      ...new Set(rawData.map((r) => r.placeId).filter(Boolean)),
    ];
    ...
    for (const placeId of uniquePlaceIds) {
      const profile = profileMap.get(placeId) as
        | {
            id: string;
            placeId: string | null;
            lastReviewDate: Date | null;
      ...
      console.log(
        `Processing ${placeReviews.length} reviews for place ${placeId}`,
      );
      ...
      for (const review of placeReviews) {
        ...
        if (
          ...
          reviewDate &&
          reviewDate <= profile.lastReviewDate
        ) {
          reviewsDuplicate++;
          continue;
        }
        ...
        const existingReview = await prisma.googleReview.findFirst({
          where: {
            ...
            reviewerId:
              review.reviewerId ||
              review.userId ||
              `reviewer-${Math.random().toString(36).substr(2, 9)}`,
          },
        });
        ...
        if (existingReview) {
          ...
          const updateData: Prisma.GoogleReviewUpdateInput = {
            ...
            textTranslated:
              review.textTranslated || existingReview.textTranslated,
            ...
            responseFromOwnerText:
              ...
              existingReview.responseFromOwnerText,
            reviewImageUrls:
              review.reviewImageUrls || existingReview.reviewImageUrls,
            scrapedAt: new Date(),
          };
    ...
    if (reviewsNew > 0) {
      ...
      const negativeReviews = newReviewsData.filter(
        (r) => (r.stars || r.rating || 0) <= 2,
      );
      ...
      const urgentReviews = newReviewsData.filter(
        (r) => (r.stars || r.rating || 0) <= 2,
      );
  ...
  private async processFacebookReviews(
    ...
    const uniqueUrls = [
      ...new Set(rawData.map((r) => r.facebookUrl).filter(Boolean)),
    ];
    ...
    for (const facebookUrl of uniqueUrls) {
      const profile = profileMap.get(facebookUrl) as
        | {
            id: string;
            facebookUrl: string | null;
            lastReviewDate: Date | null;
      ...
      console.log(
        `Processing ${pageReviews.length} reviews for page ${facebookUrl}`,
      );
      ...
      for (const review of pageReviews) {
        ...
        const reviewMetadata =
          ...
          (await prisma.reviewMetadata.create({
            data: {
              ...
              authorImage:
                review.user?.profilePic || review.userProfilePic || null,
        ...
        if (existingReview) {
          ...
          const createData: Prisma.FacebookReviewCreateInput = {
            ...
            userProfileUrl:
              review.user?.profileUrl || review.userProfileUrl || null,
            userProfilePic:
              review.user?.profilePic || review.userProfilePic || null,
            likesCount: review.likesCount || 0,
  ...
  private async processTripAdvisorReviews(
    ...
    for (const review of rawData) {
      const reviewDate = review.publishedDate
      ...
      if (
        !isInitial &&
        businessProfile.lastReviewDate &&
        reviewDate &&
        reviewDate <= businessProfile.lastReviewDate
      ) {
        reviewsDuplicate++;
        continue;
      }
      ...
      const reviewMetadata =
        ...
        (await prisma.reviewMetadata.create({
          data: {
            ...
            authorImage:
              review.user?.photoUrl || review.reviewerPhotoUrl || null,
      ...
      if (existingReview) {
        ...
        const updateData: Prisma.TripAdvisorReviewUpdateInput = {
          ...
          responseFromOwnerDate: review.responseFromOwnerDate
            ...
            : existingReview.responseFromOwnerDate,
          hasOwnerResponse:
            review.hasOwnerResponse || existingReview.hasOwnerResponse,
          scrapedAt: new Date(),
          updatedAt: new Date(),
        };
        ...
        const createData: Prisma.TripAdvisorReviewCreateInput = {
          ...
          reviewerId:
            ...
            `reviewer-${Math.random().toString(36).substr(2, 9)}`,
          reviewerName:
            review.user?.username || review.reviewerName || "Anonymous",
          reviewerLocation:
            review.user?.location || review.reviewerLocation || null,
    ...
    if (reviewsNew > 0) {
      ...
      await this.tripAdvisorAnalytics.processReviewsAndUpdateDashboard(
        businessProfile.id,
      );
    }
  ...
  private async processBookingReviews(
    ...
    if (reviewsNew > 0) {
      ...
      await this.bookingAnalytics.processReviewsAndUpdateDashboard(
        businessProfile.id,
      );
    }
  ...
  private getLatestReviewDate(rawData: any[]): Date | null {
    // External untyped data from Apify
