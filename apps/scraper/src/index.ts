...
import { BackendApiController } from './core/api/controllers/BackendApiController';
import { HealthCheckResponse, TaskStatusResponse } from './core/api/dto/ApiResponse';
...
let backendController: BackendApiController;
...
async function initializeServices(): Promise<void> {
  try {
    ...
    backendController = new BackendApiController(container);
...
app.get('/health', (_req: Request, res: Response) => {
  try {
    const servicesStatus = {
      ...
      taskController: taskController ? 'ready' : 'initializing',
      backendController: backendController ? 'ready' : 'initializing'
    };
    
    const allServicesReady = serviceFactory && businessController && reviewController && analyticsController && taskController && backendController;
...
// =================== TIKTOK BUSINESS ENDPOINTS ===================

/**
 * TikTok business profile endpoints
 * Follows Single Responsibility Principle (SRP) - delegates to appropriate controller
 */
app.post('/api/tiktok/profile', async (req: Request, res: Response) => {
  req.body.platform = MarketPlatform.TIKTOK;
  await businessController.handleRequest(req, res);
});

app.get('/api/tiktok/:teamId', async (req: Request, res: Response) => {
  req.query.platform = MarketPlatform.TIKTOK;
  await businessController.handleRequest(req, res);
});

app.put('/api/tiktok/:teamId', async (req: Request, res: Response) => {
  req.body.platform = MarketPlatform.TIKTOK;
  await businessController.handleRequest(req, res);
});

app.delete('/api/tiktok/:teamId', async (req: Request, res: Response) => {
  req.body.platform = MarketPlatform.TIKTOK;
  await businessController.handleRequest(req, res);
});

/**
 * TikTok snapshots endpoints
 * Follows Single Responsibility Principle (SRP) - delegates to appropriate controller
 */
app.post('/api/tiktok/snapshots', async (req: Request, res: Response) => {
  req.body.platform = MarketPlatform.TIKTOK;
  await reviewController.handleRequest(req, res);
});

app.get('/api/tiktok/:teamId/snapshots', async (req: Request, res: Response) => {
  req.query.platform = MarketPlatform.TIKTOK;
  await reviewController.handleRequest(req, res);
});

app.get('/api/tiktok/:teamId/analytics', async (req: Request, res: Response) => {
  req.query.platform = MarketPlatform.TIKTOK;
  await analyticsController.handleRequest(req, res);
});
...
/**
 * Main backend endpoints for complete data flow
 * Follows Single Responsibility Principle (SRP) - delegates to BackendApiController
 */
app.post('/api/backend/:teamId/:platform', async (req: Request, res: Response) => {
  await backendController.handleRequest(req, res);
});

app.get('/api/backend/:teamId/:platform', async (req: Request, res: Response) => {
  await backendController.handleRequest(req, res);
});

app.put('/api/backend/:teamId/:platform', async (req: Request, res: Response) => {
  await backendController.handleRequest(req, res);
});

app.post('/api/backend/batch', async (req: Request, res: Response) => {
  await backendController.handleRequest(req, res);
});
...
async function startServer(): Promise<void> {
  ...
  try {
    ...
    app.listen(PORT, '0.0.0.0', () => {
      ...
      console.log('');
      console.log('🚀 Main Backend endpoints (Complete Data Flow):');
      console.log(`📊 Process data: POST http://localhost:${PORT}/api/backend/:teamId/:platform?action=process`);
      console.log(`📋 Get data: GET http://localhost:${PORT}/api/backend/:teamId/:platform?action=get`);
      console.log(`🔄 Refresh data: PUT http://localhost:${PORT}/api/backend/:teamId/:platform?action=refresh`);
      console.log(`📦 Batch process: POST http://localhost:${PORT}/api/backend/batch`);
      console.log('');
      ...
      console.log('');
      console.log('🎵 TikTok endpoints:');
      console.log(`🏢 Profile: POST http://localhost:${PORT}/api/tiktok/profile`);
      console.log(`📸 Snapshots: POST http://localhost:${PORT}/api/tiktok/snapshots`);
      console.log(`📋 Get profile: GET http://localhost:${PORT}/api/tiktok/:teamId`);
      console.log(`📝 Get snapshots: GET http://localhost:${PORT}/api/tiktok/:teamId/snapshots`);
      console.log(`📊 Analytics: GET http://localhost:${PORT}/api/tiktok/:teamId/analytics`);
      console.log('');
