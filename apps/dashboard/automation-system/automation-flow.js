...
function getNextScheduledProfile(isImmediateRun = false) {
  try {
    ...
    if (isImmediateRun) {
      // For immediate runs, ignore cooldowns but still exclude failed profiles
      ...
      // For regular runs, check availability and cooldowns - NO RETRIES for failed profiles
      availableProfiles = currentSchedule.profiles.filter(p => 
        !p.completed && 
        p.status === 'scheduled' && 
        new Date(p.scheduledTime) <= now &&
  ...
  try {
    ...
    for (const selector of loginIndicators) {
      try {
        ...
        if (element) {
          log(`Login indicator found: ${selector}`, 'SYSTEM');
          return true;
        }
      } catch (err) {
        continue;
    ...
    if (url.includes('/login') || url.includes('/i/flow/login') || url.includes('/i/flow/signup')) {
      log(`Login redirect detected in URL: ${url}`, 'SYSTEM');
      return false;
    }
    
    // Additional check: try to find any tweet content
    try {
      const tweetElement = await page.$('article[data-testid="tweet"]');
      if (tweetElement) {
        log('Tweet content found - likely logged in', 'SYSTEM');
        return true;
      }
    } catch (err) {
    ...
    log('No login indicators found - user may not be logged in', 'SYSTEM');
    return false;
  } catch (err) {
    log(`Login check error: ${err.message}`, 'SYSTEM');
    return false;
  }
}
...
async function automateTwitterWithAI(profile, actionType = null, options = {}) {
  ...
  try {
    ...
    // Always start with home page for human-like behavior
    let navigationUrl = 'https://x.com/home';
    log(`Starting with home page for human-like behavior`, profileId);
    ...
    // Check if logged in BEFORE performing any actions
    const isLoggedIn = await checkLoginStatus(page);
    if (!isLoggedIn) {
      throw new Error('Not logged in to X.com. Please log in manually in AdsPower profile.');
    }
    log('Login status verified - proceeding with actions.', profileId);
    ...
    // Navigate to target URL if provided (after human-like behavior)
    if (options.targetUrl) {
      log(`Navigating to target URL: ${options.targetUrl}`, profileId);
      try {
        await page.goto(options.targetUrl, { 
          waitUntil: 'networkidle2',
          timeout: 30000 // 30 seconds timeout
        });
        log(`Successfully navigated to target URL: ${page.url()}`, profileId);
        await sleep(3000); // Wait for page to load
      } catch (error) {
        log(`Failed to navigate to target URL: ${error.message}`, profileId);
        throw new Error(`Failed to navigate to target URL: ${error.message}`);
      }
    }
...
function resetCompletedProfiles() {
  try {
    ...
    scheduleData.profiles.forEach(profile => {
      if (profile.completed || profile.status === 'completed') {
        // Reset only completed profiles - leave failed profiles as failed
...
log(`DEBUG: Received arguments: ${JSON.stringify(args)}`, 'SYSTEM');

for (let i = 0; i < args.length; i++) {
  if (args[i] === '--action' && i + 1 < args.length) {
    specifiedAction = args[i + 1];
    log(`DEBUG: Parsed action: ${specifiedAction}`, 'SYSTEM');
  } else if (args[i] === '--url' && i + 1 < args.length) {
    targetUrl = args[i + 1];
    log(`DEBUG: Parsed URL: ${targetUrl}`, 'SYSTEM');
  } else if (args[i] === '--profile' && i + 1 < args.length) {
    profileId = args[i + 1];
    log(`DEBUG: Parsed profile: ${profileId}`, 'SYSTEM');
  } else if (args[i] === '--delay' && i + 1 < args.length) {
    delay = parseInt(args[i + 1]);
    log(`DEBUG: Parsed delay: ${delay}`, 'SYSTEM');
  } else if (args[i] === '--comment' && i + 1 < args.length) {
    customComment = args[i + 1];
    log(`DEBUG: Parsed comment: ${customComment}`, 'SYSTEM');
  }
}
...
if (initializeProfiles()) {
  // TEMPORARILY DISABLED SCHEDULING FOR MANUAL TESTING
  // initializeScheduling();
  
  log('Multi-profile AI Twitter automation system started successfully!');
  log('- SCHEDULER DISABLED FOR MANUAL TESTING');
  log('- Only manual execution allowed');
  
  // Only run immediately if this is the main automation script, not when imported by Telegram bot
  if (process.argv[1] && process.argv[1].includes('automation-flow.js')) {
    if (specifiedAction) {
      ...
      if (targetProfile) {
        ...
        if (targetUrl) {
          ...
          options.targetSpecificTweet = true; // Enable specific tweet targeting
          log(`DEBUG: Target URL received: ${targetUrl}`, 'SYSTEM');
          log(`DEBUG: targetSpecificTweet set to: ${options.targetSpecificTweet}`, 'SYSTEM');
        } else {
          log(`DEBUG: No target URL provided`, 'SYSTEM');
        }
