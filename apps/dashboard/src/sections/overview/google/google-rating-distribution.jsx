...
export function GoogleRatingDistribution({ businessProfile, currentPeriodMetrics, sx, ...other }) {
  const theme = useTheme();

  // Generate rating distribution data using the same logic as GoogleRatingDistribution
  const ratingDistributionData = useMemo(() => {
    ...
    if (
      ...
      businessProfile.reviews.forEach((review) => {
        const rating = Math.floor(review.stars || review.rating || 0);
    ...
    // Fallback 2: Use reviewsDistribution from business profile if available
    if (businessProfile?.reviewsDistribution) {
      const dist = businessProfile.reviewsDistribution;
      return [
    ...
    if (businessProfile) {
      ...
      if (avgRating >= 4.5) {
        // Excellent business - mostly 5 and 4 stars
        distribution = {
        ...
        // Good business - mostly 4 and 5 stars with some 3s
        distribution = {
        ...
        // Average business - mixed reviews
        distribution = {
