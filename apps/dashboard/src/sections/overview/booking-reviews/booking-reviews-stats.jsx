...
export function BookingReviewsStats({ stats = {} }) {
  const theme = useTheme();

  // Generate mock chart data for demonstration (currently unused)
  ...
  const statsData = [
    ...
    },
    {
      ...
      total: `${stats.averageRating ? stats.averageRating.toFixed(1) : '0.0'} / 10`,
    ...
    },
    {
      ...
      total: `${stats.verifiedStays || 0}`,
      subtitle:
        stats.total > 0
          ? `${(((stats.verifiedStays || 0) / stats.total) * 100).toFixed(1)}% of total`
          : '0% of total',
      icon: <Iconify icon="solar:shield-check-bold" width={24} />,
      color: 'success',
      // percent: 5,
      showProgress: true,
      progressValue: stats.total > 0 ? ((stats.verifiedStays || 0) / stats.total) * 100 : 0,
      // chart: {
      //   series: generateMockChartData(),
      //   categories: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
      // },
