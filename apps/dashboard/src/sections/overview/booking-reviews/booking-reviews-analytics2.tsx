...
import { useBookingReviews, useBookingEnhancedGraph, useUpdateBookingReviewMetadata } from 'src/hooks/useLocations';
...
interface BookingReviewsAnalytics2Props {
  teamSlug: string;
  locationId?: string;
  title?: string;
  subheader?: string;
...
function ReviewCard({ review, searchTerm, onImageClick, onUpdateMetadata }: ReviewCardProps) {
  ...
  // Convert Booking.com 1-10 rating to percentage for display
  const getProgressValue = (rating: number | null) => {
    if (!rating) return 0;
      ...
      <CardContent sx={{ p: 3, position: 'relative' }}>
        <Stack spacing={2}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 2 }}>
            ...
            <Box sx={{ flex: 1 }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 1 }}>
                ...
                {review.guestType && (
                  <Chip label={review.guestType} size="small" variant="outlined" />
                )}
                {review.roomType && (
          ...
          {review.likedMost && (
            ...
            >
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                <Iconify icon="eva:checkmark-circle-2-fill" className="" height={16} sx={{ color: 'success.main' }} />
                <Typography variant="caption" sx={{ fontWeight: 600, color: 'success.main' }}>
                  Positive
                </Typography>
              </Box>
              <Typography variant="body2" sx={{ lineHeight: 1.6 }}>
                {highlightSearchTerms(review.likedMost, searchTerm)}
              </Typography>
            </Box>
          )}
          ...
          {review.dislikedMost && (
            ...
            >
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                <Iconify icon="eva:close-circle-fill" className="" height={16} sx={{ color: 'error.main' }} />
                <Typography variant="caption" sx={{ fontWeight: 600, color: 'error.main' }}>
                  Negative
                </Typography>
              </Box>
              <Typography variant="body2" sx={{ lineHeight: 1.6 }}>
                {highlightSearchTerms(review.dislikedMost, searchTerm)}
              </Typography>
            </Box>
          )}
          ...
          {/* Sub-ratings */}
          {(review.cleanlinessRating || review.comfortRating || review.locationRating || review.facilitiesRating) && (
            <Box>
              <Typography variant="caption" color="text.secondary" sx={{ mb: 1, display: 'block' }}>
                Sub-ratings
              </Typography>
              <Stack spacing={0.5}>
                {review.cleanlinessRating && (
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    <Typography variant="caption" sx={{ minWidth: 80 }}>Cleanliness:</Typography>
                    <LinearProgress
                      variant="determinate"
                      value={getProgressValue(review.cleanlinessRating)}
                      sx={{ flex: 1, height: 6, borderRadius: 3 }}
                    />
                    <Typography variant="caption" sx={{ minWidth: 30, textAlign: 'right' }}>
                      {review.cleanlinessRating.toFixed(1)}
                    </Typography>
                  </Box>
                )}
                {review.comfortRating && (
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    <Typography variant="caption" sx={{ minWidth: 80 }}>Comfort:</Typography>
                    <LinearProgress
                      variant="determinate"
                      value={getProgressValue(review.comfortRating)}
                      sx={{ flex: 1, height: 6, borderRadius: 3 }}
                    />
                    <Typography variant="caption" sx={{ minWidth: 30, textAlign: 'right' }}>
                      {review.comfortRating.toFixed(1)}
                    </Typography>
                  </Box>
                )}
                {review.locationRating && (
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    <Typography variant="caption" sx={{ minWidth: 80 }}>Location:</Typography>
                    <LinearProgress
                      variant="determinate"
                      value={getProgressValue(review.locationRating)}
                      sx={{ flex: 1, height: 6, borderRadius: 3 }}
                    />
                    <Typography variant="caption" sx={{ minWidth: 30, textAlign: 'right' }}>
                      {review.locationRating.toFixed(1)}
                    </Typography>
                  </Box>
                )}
                {review.facilitiesRating && (
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    <Typography variant="caption" sx={{ minWidth: 80 }}>Facilities:</Typography>
                    <LinearProgress
                      variant="determinate"
                      value={getProgressValue(review.facilitiesRating)}
                      sx={{ flex: 1, height: 6, borderRadius: 3 }}
                    />
                    <Typography variant="caption" sx={{ minWidth: 30, textAlign: 'right' }}>
                      {review.facilitiesRating.toFixed(1)}
                    </Typography>
                  </Box>
                )}
              </Stack>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', pt: 1 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              {review.reviewerNationality && (
                <Chip label={review.reviewerNationality} size="small" variant="outlined" />
              )}
              {review.lengthOfStay && (
                <Typography variant="caption" color="text.secondary">
            ...
            <Box sx={{ display: 'flex', gap: 1 }}>
              {/* Booking reviews don't have a direct review URL */}
            </Box>
          </Box>
        </Stack>
      </CardContent>
    ...
    if (sentiment && sentiment !== 'all') {
      filtered = filtered.filter((review) => {
        ...
        if (reviewSentiment === null || reviewSentiment === undefined) {
          ...
          switch (sentiment) {
            case 'positive':
              return normalizedScore >= 4;
            case 'negative':
              return normalizedScore <= 2;
            case 'neutral':
              return normalizedScore > 2 && normalizedScore < 4;
            default:
              return true;
          }
        }
...
export function BookingReviewsAnalytics2({ 
  ...
  const { daily: analyticsData, isLoading: isFetchingAnalytics, error: analyticsError } = useBookingEnhancedGraph(locationId, startDate, endDate);

  // Helper function to aggregate data by periods (similar to Google, Facebook, and TripAdvisor)
