...
export function BookingMetricsWidgetSummary({
  sx,
  icon,
  title,
  total,
...
export function BookingMetricsOverview({ metrics, periodicalMetrics, currentPeriodKey }) {
  const theme = useTheme();
  ...
  const generateChartDataFromPeriods = (metricKey) => {
    ...
    return sortedPeriods.map((period) => {
      switch (metricKey) {
        ...
        case 'responseRate':
          return Number(period.responseRate) || 0;
        case 'averageLengthOfStay':
          return Number(period.averageLengthOfStay) || 0;
        case 'totalReviews':
  ...
  const getCurrentPeriodData = (metricKey) => {
    ...
    switch (metricKey) {
      ...
      case 'responseRate':
        return currentPeriod.responseRate;
      case 'averageLengthOfStay':
        return currentPeriod.averageLengthOfStay;
      case 'totalReviews':
  ...
  const calculatePercentageChange = (metricKey) => {
    ...
    switch (metricKey) {
      case 'averageRating':
        ...
        break;
      case 'responseRate':
        currentValue = Number(currentPeriod.responseRate) || 0;
        previousValue = Number(previousPeriod.responseRate) || 0;
        break;
      case 'averageLengthOfStay':
        currentValue = Number(currentPeriod.averageLengthOfStay) || 0;
        previousValue = Number(previousPeriod.averageLengthOfStay) || 0;
        break;
      case 'totalReviews':
  ...
  const metricCards = [
    ...
    },
    {
      ...
      total: getCurrentPeriodData('averageLengthOfStay')
        ? `${getCurrentPeriodData('averageLengthOfStay')} nights`
        : 'N/A',
      icon: <Iconify icon="solar:calendar-bold" width={24} />,
      color: 'success',
      percent: calculatePercentageChange('averageLengthOfStay'),
      chart: {
        series: generateChartDataFromPeriods('averageLengthOfStay'),
        categories,
      },
    },
    {
