...
import { BookingReviewsList } from '../components/booking-reviews-list';
import { BookingReviewsStats } from '../components/booking-reviews-stats';
import { BookingReviewsFilters } from '../components/booking-reviews-filters';
import { BookingReviewsWelcome } from '../components/booking-reviews-welcome';
import { BookingReviewsLoadingSkeleton } from '../components/booking-reviews-loading-skeleton';

// Lazy load the heavy analytics component
const BookingReviewsAnalytics = lazy(() => import('../components/booking-reviews-analytics').then(module => ({ default: module.BookingReviewsAnalytics })));
...
export function TeamBookingReviewsView() {
  ...
  const hookFilters = useMemo(() => ({
    ...
    sortBy: (filters.sortBy === 'rating' ? 'rating' : 
            filters.sortBy === 'responseStatus' ? 'hasResponse' : 
            filters.sortBy) as 'publishedDate' | 'rating' | 'hasResponse'
  }), [filters]);
  ...
  const handleUpdateMetadata = useCallback(async (reviewId: string, updates: any) => {
    try {
      ...
      const response = await fetch(`/api/teams/${slug}/booking/reviews/${reviewId}/metadata`, {
        method: 'PATCH',
      ...
      <DashboardContent maxWidth="xl" sx={{}} className="" disablePadding={false}>
        <Card sx={{ p: 3, textAlign: 'center' }}>
          <Typography variant="h6" color="text.secondary">
            Team or Booking.com Profile not found
          </Typography>
        </Card>
      </DashboardContent>
    );
