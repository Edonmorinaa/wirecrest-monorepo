...
const LogoZoomPrimaryOutline = styled(m.span)(({ theme }) => {
  const color = theme.vars?.palette?.primary?.darkChannel || theme.palette.primary.darkChannel;
  return {
    position: 'absolute',
    width: 'calc(100% - 20px)',
    height: 'calc(100% - 20px)',
    border: color ? `solid 3px ${varAlpha(color, 0.24)}` : `solid 3px ${theme.palette.primary.dark}24`,
  };
});
...
const LogoZoomSecondaryOutline = styled(m.span)(({ theme }) => {
  ...
  return {
    ...
    height: '100%',
    position: 'absolute',
    border: color ? `solid 8px ${varAlpha(color, 0.24)}` : `solid 8px ${theme.palette.primary.dark}24`,
  };
});
