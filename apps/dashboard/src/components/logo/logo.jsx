...
export function Logo({ sx, disabled, className, href = '/', isSingle = true, ...other }) {
  ...
  // Add fallbacks for when theme is not available
  const TEXT_PRIMARY = theme?.vars?.palette?.text?.primary || '#000000';
  const PRIMARY_LIGHT = theme?.vars?.palette?.primary?.light || '#1976d2';
  const PRIMARY_MAIN = theme?.vars?.palette?.primary?.main || '#1976d2';
  const PRIMARY_DARKER = theme?.vars?.palette?.primary?.dark || '#1565c0';
