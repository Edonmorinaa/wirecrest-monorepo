...
export const deleteInvitation = async (key: { token: string } | { id: string }) => prisma.invitation.delete({
  where: key,
});

export const isInvitationExpired = async (expires: Date) => expires.getTime() < Date.now();

export const getInvitationCount = async ({ where }) => prisma.invitation.count({
  where,
});
