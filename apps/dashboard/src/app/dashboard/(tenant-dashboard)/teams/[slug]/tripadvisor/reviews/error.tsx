...
export default function Error({ error, reset }: ErrorProps) {
  ...
  const getErrorMessage = () => {
    ...
    if (error.message.includes('PAYMENT_REQUIRED')) {
      return 'An active subscription is required to access TripAdvisor Reviews.';
    }
    if (error.message.includes('PRECONDITION_FAILED')) {
      return 'Your subscription plan does not include TripAdvisor Reviews. Please upgrade.';
    }
    if (error.message.includes('NOT_FOUND')) {
      return 'TripAdvisor profile not found. Please connect your TripAdvisor account.';
    }
    return error.message || 'Failed to load TripAdvisor Reviews';
  };
