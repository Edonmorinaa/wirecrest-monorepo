import NextAuth, { type Account, type Profile, type User, type Session } from 'next-auth';
...
const authConfig: any = {
  ...
  callbacks: {
    ...
    async session({ session, token, user }: { session: any; token: any; user?: User }) {
      ...
      // Ensure session matches auth-core Session interface
      if (session && (token || user)) {
        ...
        // Get superRole, role, teamId, team from token (for JWT) or user (for database sessions)
        if (token?.superRole) {
        ...
        session.user.role = token?.role || (user as any)?.role;
    ...
    async jwt({ token, user, account, trigger, session }: { token: any; user?: User; account?: Account | null; trigger?: string; session?: any }) {
      ...
      if (token.sub && (!token.superRole || !token.teamId)) {
        ...
        if (dbUser) {
          ...
          token.picture = dbUser.image; // NextAuth uses 'picture' for images
          ...
          if (primaryTeamMember) {
            ...
            token.role = primaryTeamMember.team.defaultRole; // Store team role
            token.team = {
