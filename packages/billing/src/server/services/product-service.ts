...
import { ProductData, SubscriptionTier, StripeTierMetadata, TierMetadata, parseTierMetadata } from '../../shared/types/index.js';
...
export class ProductService {
  ...
  async createSubscriptionProducts(): Promise<void> {
    ...
    for (const config of tierConfigs) {
      try {
        ...
        // Update the tier config with Stripe IDs
        await this.updateTierConfigWithStripeIds(config.id, stripeProduct.id, stripePrice.id);
  ...
  /**
   * Update tier config with Stripe product and price IDs
   */
  private async updateTierConfigWithStripeIds(
    configId: string,
    ...
    console.log(`✅ Updated tier config with Stripe IDs: ${stripeProductId}, ${stripePriceId}`);
  }
